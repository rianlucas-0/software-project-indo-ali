<footer class="bg-gray-950 text-white pt-12 pb-20 md:pt-16 md:pb-8">
    <div class="container mx-auto px-5 max-w-7xl">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-8 md:gap-10 mb-8 md:mb-10">
            ...
            <div class="order-3 md:order-2 pt-6 md:pt-0 border-t md:border-t-0 border-gray-800">
                ...
                <ul class="space-y-3">
                    <li>
                        <a href="/about" class="text-gray-400 hover:text-white transition flex items-center group">
                            <span
            ...
            <div class="order-4 md:order-3 pt-6 md:pt-0 border-t md:border-t-0 border-gray-800">
                ...
                <ul class="space-y-3">
                    <li class="text-gray-400 hover:text-white transition flex items-center cursor-pointer"
                        id="openSupportChat">
        ...
        <div class="pt-6 md:pt-8 border-t border-gray-800 mt-6 md:mt-0">
            <div class="flex flex-col md:flex-row justify-between items-center gap-3 md:gap-4">
                ...
                <div class="hidden md:flex gap-4 md:gap-6">
                    <a href="{{ route('policies.privacy') }}"
                        class="text-gray-400 hover:text-white text-sm transition">Política de Privacidade</a>
                    <a href="{{ route('policies.cookies') }}"
                        class="text-gray-400 hover:text-white text-sm transition">Política de Cookies</a>
                    <a href="{{ route('policies.consent') }}"
                        class="text-gray-400 hover:text-white text-sm transition">Termo de Consentimento</a>
                </div>
            </div>
        </div>
    </div>
    ...
    <div id="supportChatModal"
        ...
        <!-- Modal -->
        <div
            class="fixed bottom-0 left-0 right-0 md:relative md:rounded-2xl bg-[#161B22] border border-gray-700 shadow-2xl flex flex-col h-[85vh] md:h-[600px] md:max-h-[80vh] transform transition-transform duration-300">
            <!-- Cabeçalho -->
            ...
            <!-- Área de Conteúdo -->
            <div class="flex-1 flex flex-col overflow-hidden bg-[#1E2229]">
                <!-- Mensagens -->
                <div class="flex-1 overflow-y-auto p-4 space-y-4">

                    <div class="flex justify-start">
                        <div class="bg-[#161B22] rounded-2xl rounded-tl-none p-4 max-w-[85%] md:max-w-[80%]">
                            <div class="font-medium text-indigo-400 text-sm mb-1">Suporte Indo Ali</div>
                            <div class="text-gray-300 text-sm">Olá! Como posso ajudá-lo hoje?</div>
                        </div>
                    </div>

                    <div class="bg-[#161B22] rounded-2xl p-4">
                        <label for="supportTopic" class="block mb-3 font-medium text-sm text-gray-300">Selecione o
                            problema</label>
                        <select id="supportTopic"
                            class="w-full p-3 bg-[#1E2229] border border-gray-700 rounded-lg text-sm text-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
                            <option value="" class="text-gray-500">— Escolha um tópico —</option>
                            <option value="login" class="text-white">Problemas para entrar / login</option>
                            <option value="upload" class="text-white">Erro ao enviar fotos / upload</option>
                            <option value="bug_ui" class="text-white">Bug na interface</option>
                            <option value="perf" class="text-white">Site lento / performance</option>
                            <option value="account" class="text-white">Alterar dados / conta</option>
                            <option value="other" class="text-white">Outro problema técnico</option>
                        </select>
                    </div>
                ...
                <div class="p-4 border-t border-gray-700 bg-[#161B22]">
                    <div class="flex flex-col sm:flex-row gap-3">
                        ...
                        <button id="supportHuman"
                            class="flex-1 bg-gray-700 hover:bg-gray-600 text-white py-3 px-4 rounded-lg text-sm font-medium transition flex items-center justify-center order-1 sm:order-2">
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        select.addEventListener('change', () => {
            ...
            assistantMsg.innerHTML = `
                    <div class="bg-[#161B22] rounded-2xl rounded-tl-none p-4 max-w-[85%] md:max-w-[80%]">
                        <div class="font-medium text-indigo-400 text-sm mb-1">Assistente</div>
                        <div class="text-gray-300 text-sm">${node.answer}</div>
                    </div>
                `;
            ...
            node.followups.forEach(f => {
                ...
                button.type = 'button';
                button.className =
                    'w-full text-left p-3 border border-gray-700 rounded-lg bg-[#161B22] hover:bg-gray-800 text-sm text-gray-300 transition duration-200';
                ...
                button.addEventListener('click', () => {
                    optionsContainer.querySelectorAll('button').forEach(btn => {
                        btn.classList.remove('border-indigo-500',
                            'bg-indigo-900/20', 'text-white');
                    });

                    button.classList.add('border-indigo-500', 'bg-indigo-900/20',
                        'text-white');
            ...
            if (node.followups.some(f => f.id === 'custom')) {
                ...
                textarea.className =
                    'w-full p-3 bg-[#161B22] border border-gray-700 rounded-lg text-sm text-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition resize-none';
                textarea.rows = 4;
        ...
        function generateTechReply(sel) {
            ...
            const responses = {
                login: {
                    wrong_pass: '<div class="font-medium text-indigo-400 mb-2">Senha incorreta</div><div class="text-sm">Tente redefinir a senha em "Esqueci minha senha". Se já redefiniu e não funciona, limpe cache/cookies ou tente usar outra aba anônima.</div>',
                    no_account: '<div class="font-medium text-indigo-400 mb-2">Conta não encontrada</div><div class="text-sm">Verifique se você cadastrou com o e-mail correto. Se quiser, podemos enviar um link para criar conta.</div>',
                    oauth: '<div class="font-medium text-indigo-400 mb-2">Login social</div><div class="text-sm">Confirme que a conta Google/Facebook está autenticada no navegador e que popups não estão bloqueados.</div>'
                },
                upload: {
                    file_too_large: '<div class="font-medium text-indigo-400 mb-2">Arquivo grande</div><div class="text-sm">Tente reduzir a resolução/qualidade ou enviar imagens abaixo de 5MB. Estamos planejando aumentar limite.</div>',
                    replace: '<div class="font-medium text-indigo-400 mb-2">Imagem substituída</div><div class="text-sm">Esse é um bug conhecido. Como solução alternativa, selecione múltiplas imagens ao mesmo tempo.</div>',
                    format: '<div class="font-medium text-indigo-400 mb-2">Formato</div><div class="text-sm">Aceitamos JPG e PNG. Converta para um desses formatos e tente novamente.</div>'
                },
                bug_ui: {
                    button: '<div class="font-medium text-indigo-400 mb-2">Botão não responde</div><div class="text-sm">Tente atualizar a página (Ctrl+F5). Se ocorrer apenas em mobile, informe o modelo e versão do navegador.</div>',
                    layout: '<div class="font-medium text-indigo-400 mb-2">Layout quebrado</div><div class="text-sm">Descreva o dispositivo e a resolução. Informe também se está em modo mobile/desktop.</div>',
                    images: '<div class="font-medium text-indigo-400 mb-2">Imagens não carregam</div><div class="text-sm">Verifique sua conexão. Se o problema persistir, vamos coletar logs e investigar.</div>'
                },
                perf: {
                    slow_page: '<div class="font-medium text-indigo-400 mb-2">Página lenta</div><div class="text-sm">Tente limpar cache. Se repetir, ao escalar iremos coletar tempo de carregamento e logs.</div>',
                    timeout: '<div class="font-medium text-indigo-400 mb-2">Timeout / 504</div><div class="text-sm">Isso indica problema no servidor backend. Iremos abrir ticket e investigar.</div>',
                    api: '<div class="font-medium text-indigo-400 mb-2">Busca lenta</div><div class="text-sm">Relate qual busca e filtros você utilizou; isso ajuda a replicar o problema.</div>'
                },
                account: {
                    change_email: '<div class="font-medium text-indigo-400 mb-2">Mudar e-mail</div><div class="text-sm">Você pode alterar no perfil. Se não conseguir, escalaremos para verificação manual.</div>',
                    reset_password: '<div class="font-medium text-indigo-400 mb-2">Redefinir senha</div><div class="text-sm">Clique em "Esqueci minha senha" e siga o e-mail. Se não chegar, verifique spam.</div>',
                    delete_account: '<div class="font-medium text-indigo-400 mb-2">Excluir conta</div><div class="text-sm">Envie confirmação por e-mail. Atenção: exclusão é irreversível.</div>'
                },
                other: {
                    custom: `<div class="font-medium text-indigo-400 mb-2">Recebemos sua descrição</div><div class="text-sm mb-2">${escapeHtml(label)}</div><div class="text-xs text-gray-500">Se precisar de mais ajuda, clique em "Falar com humano".</div>`
                }
            };
        ...
        confirmBtn.addEventListener('click', () => {
            ...
            assistantMsg.innerHTML = `
                <div class="bg-[#161B22] rounded-2xl rounded-tl-none p-4 max-w-[85%] md:max-w-[80%]">
                    <div class="font-medium text-indigo-400 text-sm mb-1">Assistente</div>
                    <div class="text-gray-300 text-sm">${generateTechReply(sel)}</div>
                </div>
            `;
            followup.appendChild(assistantMsg);
        ...
        humanBtn.addEventListener('click', () => {
            ...
            humanBtn.classList.remove('bg-gray-700', 'hover:bg-gray-600');
            humanBtn.classList.add('bg-green-600', 'cursor-default');
