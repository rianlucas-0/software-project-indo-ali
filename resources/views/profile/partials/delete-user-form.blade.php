<section class="space-y-6">
    ...
    <x-modal name="confirm-user-deletion" :show="$errors->userDeletion->isNotEmpty()" focusable>
        <form method="post" action="{{ route('profile.destroy') }}"
            class="p-6 bg-[#161B22] border border-gray-700 rounded-lg">
            @csrf
            @method('delete')

            <div class="flex items-start">
                <i class="fas fa-exclamation-circle text-red-400 mr-3 text-xl mt-1"></i>
                <div>
                    <h2 class="text-lg font-medium text-white">
            ...
            <div class="mt-6">
                <div class="relative">
                    <x-text-input id="password" name="password" type="password"
                        class="block w-full bg-[#1E2229] border-gray-700 text-white focus:border-red-500 focus:ring-red-500 pl-10"
            ...
            <div class="mt-6 flex justify-end space-x-3">
                <x-secondary-button x-on:click="$dispatch('close')"
                    class="bg-gray-600 hover:bg-gray-700 focus:bg-gray-700">
                    {{ __('Cancelar') }}
                </x-secondary-button>
